...
| 11 | Public profile path | `/card/:handle` (handoff/deep-links.md) | **`/dr/{slug}`** (PRODUCT_CORE §IA, SCREENS PR-06) | precedence rule 1 — this folder beats `handoff/`. The app implements `/dr/:slug`; a `/card/*` link printed on a physical card would 404, so nothing may be printed with the old path |
| 12 | Offline downloads | LN-11 Downloads screen; offline-downloaded playback (SCREENS LN-05); wifi-only setting; keep-downloads logout option | **CUT** — owner decision 16 Aug 2026. Lessons stream, handouts open in-viewer, nothing is written to the device | Never built. The state matrix's "offline player note" gap, the wifi-only preference and the keep-downloads logout branch existed only to serve this feature and are removed with it. The offline **cache** of already-read content is unaffected and stays |
