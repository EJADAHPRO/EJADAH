...
void main() {
  ...
  setUp(() {
    ...
    handler = const Pipeline()
        ...
        .addHandler(
          (Router()..mount('/uploads', uploadRoutes(store).call)).call,
        );
  });
  ...
  group('what may be stored', () {
    test('a PNG is stored under a key naming its owner', () async {
      ...
      expect(stored.key, startsWith('document/user-a/'));
  ...
  group('who may read', () {
    ...
    test("a stranger gets 404, not 403, for someone else's certificate",
      ...
      expect(await response.readAsString(), isNot(contains('user-a')));
    });
  ...
  group('the key cannot climb out', () {
    test('a traversing key reads nothing', () async {
      ...
      expect(await store.read('document/user-a/../../secret.txt'), isNull);
    });
  ...
  group('the route', () {
    test('stores and answers with the key', () async {
      final response = await handler(
        Request(
          ...
          headers: {'content-type': 'application/json', ..._auth(tokens, 'user-a')},
      ...
      expect(body['key'], startsWith('avatar/user-a/'));
    ...
    test('an oversized body is refused before it is decoded', () async {
      // Nine megabytes of base64. The cap is checked on the encoded length so
      // this never becomes nine megabytes of Uint8List first.
      final huge = 'A' * (9 * 1024 * 1024);
      final response = await handler(
        Request(
          'POST',
          Uri.parse('http://x/uploads/document'),
